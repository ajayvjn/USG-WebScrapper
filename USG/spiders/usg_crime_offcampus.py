...
class ExampleSpider(CrawlSpider):
    ...
    def captureDataAndWrite(self, response):
        ...
        murder_manslaughter = Selector(response).xpath(
            '//*[@data-field-id="gCrimNoncampus"]/table/tbody/tr[1]/td[4]/div[1]/text()').extract()
        ...
        neg_manslaughter = Selector(response).xpath(
            '//*[@data-field-id="gCrimNoncampus"]/table/tbody/tr[2]/td[4]/div[1]/text()').extract()
        ...
        rape = Selector(response).xpath(
            '//*[@data-field-id="gCrimNoncampus"]/table/tbody/tr[3]/td[4]/div[1]/text()').extract()
        rape = rape[0].strip()
        print rape
        data["Rape"] = rape

        incest = Selector(response).xpath(
            '//*[@data-field-id="gCrimNoncampus"]/table/tbody/tr[4]/td[4]/div[1]/text()').extract()
        incest = incest[0].strip()
        print incest
        data["Incest"] = incest

        statutory_rape = Selector(response).xpath(
            '//*[@data-field-id="gCrimNoncampus"]/table/tbody/tr[5]/td[4]/div[1]/text()').extract()
        ...
        fondling = Selector(response).xpath(
            '//*[@data-field-id="gCrimNoncampus"]/table/tbody/tr[6]/td[4]/div[1]/text()').extract()
        fondling = fondling[0].strip()
        print fondling
        data["Fondling"] = fondling

        robbery = Selector(response).xpath(
            '//*[@data-field-id="gCrimNoncampus"]/table/tbody/tr[9]/td[4]/div[1]/text()').extract()
        robbery = robbery[0].strip()
        print robbery
        data["Robbery"] = robbery

        aggravated_assault = Selector(response).xpath(
            '//*[@data-field-id="gCrimNoncampus"]/table/tbody/tr[10]/td[4]/div[1]/text()').extract()
        ...
        burglary = Selector(response).xpath(
            '//*[@data-field-id="gCrimNoncampus"]/table/tbody/tr[11]/td[4]/div[1]/text()').extract()
        burglary = burglary[0].strip()
        print burglary
        data["Burglary"] = burglary

        motor_vehicle_theft = Selector(response).xpath(
            '//*[@data-field-id="gCrimNoncampus"]/table/tbody/tr[12]/td[4]/div[1]/text()').extract()
        ...
        arson = Selector(response).xpath(
            '//*[@data-field-id="gCrimNoncampus"]/table/tbody/tr[13]/td[4]/div[1]/text()').extract()
        arson = arson[0].strip()
        print arson
        data["Arson"] = arson
